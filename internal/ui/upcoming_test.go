...
func TestUpcomingDateRenderingAndDimensions(t *testing.T) {
	...
	line := renderTaskLine(task, true, false, 60, colorPaneBg, testNow(), true)
	...
	for _, width := range []int{12, 20, 30, 60} {
		if got := lipgloss.Width(renderTaskLine(task, true, false, width, colorPaneBg, testNow(), true)); got > width {
